...
def test_all_motion_npz_have_beyondmimic_keys():
    for spec in TRACKING_MOTIONS:
        path = motion_npz_path(spec.key)
...
def test_walk_motion_npz_has_beyondmimic_keys():
    ...
    assert UMR_WALK_MOTION_FILE == motion_npz_path("walk")
...
def test_jump_and_spinkick_use_looser_fail_z():
    jump = make_microduck_tracking_env_cfg(motion_key="jump")
    spin = make_microduck_tracking_env_cfg(motion_key="spinkick")
    assert jump.terminations["anchor_pos"].params["threshold"] == 0.08
    assert spin.terminations["ee_body_pos"].params["threshold"] == 0.08


def test_each_motion_has_its_own_rl_run():
    for spec in TRACKING_MOTIONS:
        rl = make_microduck_tracking_rl_cfg(spec.key)
        assert rl.experiment_name == f"tracking_{spec.key}"
        assert tracking_task_id(spec.key).endswith(spec.label)
